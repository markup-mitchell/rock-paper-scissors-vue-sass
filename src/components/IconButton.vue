<template>
  ...
  >
    <button :value="value" class="btn" @click="setPlayerChoice(value)">
      <svg
        v-if="value === 'rock'"
...
.btn {
  ...
  box-shadow: inset 0 5px 0 $shadow-inset;
}
.rock {
  background-image: $bg-rock;
  box-shadow: 0 6px 0 $shadow-rock;
}
.paper {
  background-image: $bg-paper;
  box-shadow: 0 6px 0 $shadow-paper;
}
.scissors {
  background-image: $bg-scissors;
  box-shadow: 0 6px 0 $shadow-scissors;
}
</style>
...
export default {
  ...
  methods: {
    setPlayerChoice(value) {
      // this.$parent.$emit is a workaround - vuex would be a better solution; I just don't have time to learn that right now!
      this.$parent.$emit('setPlayerChoice', value);
    }
  }
};
</script>
